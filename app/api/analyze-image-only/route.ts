...
export async function POST(request: Request) {
  try {
    ...
    if (!cleanBase64) {
      return NextResponse.json({ message: 'No image provided', status: 400, remainingRequests: 0 }, { status: 400 });
    }
    ...
    if (!modelConfig.supportsImages) {
      return NextResponse.json({ message: 'Selected model does not support image analysis', status: 400, remainingRequests: 0 }, { status: 400 });
    }
    ...
    const messages: GenericMessage[] = [
      ...
      { role: 'user', imageBase64: `data:image/jpeg;base64,${cleanBase64}`, content: '' }
    ];
